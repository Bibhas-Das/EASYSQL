...
class serverConnect():
    def __init__(self):
        ...
        if os.path.exists("database_credentials.json"):
            ...
            self.password=getpass.getpass(f"\n\tEnter Password for {self.user}: ")  # Mask password input
            self.database=self.decode_base64(database_credentials['database'])
        else:
            print("Please run saveCredential.py file for save credentials")
            os._exit(0)
